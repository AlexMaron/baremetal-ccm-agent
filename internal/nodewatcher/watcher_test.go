...
func (h *handlerSpy) Handler(_ context.Context, _, _ string, node *v1.Node, _ haproxy.API, _ HAProxyWriter) {
	h.mu.Lock()
	defer h.mu.Unlock()
	h.calls++
	if node != nil {
...
func testHandler(call *handlerCall) NodeHandlerFunc {
	return func(ctx context.Context, hostname, ip string, node *v1.Node, haproxyReader haproxy.API, haproxyWriter HAProxyWriter) {
		call.called = true
...
func TestHandleNode_normalNode(t *testing.T) {
	...
	handleNode(context.Background(), node, testHandler(call), nil, nil)
...
func TestStartWatcherWithClient_ReturnStopChannel(t *testing.T) {
	clientset := fake.NewClientset()
	stopCh := StartWatcherWithClient(context.Background(), clientset, dummyHandler, &MockHAProxyAPI{}, &MockHAProxyAPI{})
